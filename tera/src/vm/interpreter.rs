    ...
    pub(crate) fn interpret(
        ...
        while let Some((instr, _)) = state.chunk.expect("To have a chunk").get(ip) {
            ...
            match instr {
                ...
                Instruction::RunTranslationWithArgs(message)
                | Instruction::RunTranslation(message) => {
                    let translator = if let Some(t) = &self.tera.translator {
                        t
                    } else {
                        rendering_error!(
                            format!("Translation was invoked but no translator is registred!"),
                            Some(current_ip..=current_ip)
                        );
                    };
                    let res = if matches!(instr, Instruction::RunTranslationWithArgs(_)) {
                        ...
                        translator.call(message, None, state)
                    };
                    match res {
                        Ok(val) => {
                            let val = if translator.is_safe() {
                                val.mark_safe()
                            } else {
                                val
                            };
